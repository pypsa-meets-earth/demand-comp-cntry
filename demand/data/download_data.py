...
# raw
if not os.path.exists(os.path.join(project_root, r'data/raw')):
    os.makedirs(os.path.join(project_root, r'data/raw'))
...
if __name__ == "__main__":
    if "snakemake" not in globals():
        ...
        snakemake = mock_snakemake("download_data")
    ...
    target_path = os.path.join(project_root, r'data/raw/country-and-continent-codes-list-csv_csv.zip')
    ...
    target_path = os.path.join(project_root, r'data/raw/TM_WORLD_BORDERS-0.3.zip')
    data_label = "World country borders shapefile"
    download_raw_data(source_url, target_path, data_label)

    # WB Population Data
    source_url = "https://www.dropbox.com/s/22jszjd1sohk6g7/wb_pop.zip?dl=1"
    target_path = os.path.join(project_root, r'data/raw/wb_pop.zip')
    ...
    target_path = os.path.join(project_root, r'data/raw/UNdata_Export_20201006_214431107_SomaliaGDPpc.zip')
    data_label = "UN Somalia GDP Data"
    download_raw_data(source_url, target_path, data_label)

    # WB Rainfall Data
    source_url = "https://www.dropbox.com/s/500p8cthhsjlnz6/rainfall_1960-2016.zip?dl=1"
    target_path = os.path.join(project_root, r'data/raw/rainfall_1960-2016.zip')
    data_label = "WB Rainfall Data"
    download_raw_data(source_url, target_path, data_label)

    # WB Temperature Data
    source_url = "https://www.dropbox.com/s/d7jfz4laab51g4h/temperature_1960-2016.zip?dl=1"
    target_path = os.path.join(project_root, r'data/raw/temperature_1960-2016.zip')
    ...
    target_path = os.path.join(project_root, r'data/raw/API_VC.BTL.DETH_DS2_en_csv_v2_2167203.zip')
    data_label = "WB Battle-Related Deaths Data"
    download_raw_data(source_url, target_path, data_label)

    # IEA CDD
    source_url = "https://www.dropbox.com/s/ckum91no8z71t7r/CDD_18_IEA_20210406.zip?dl=1"
    target_path = os.path.join(project_root, r'data/raw/CDD_18_IEA_20210406.zip')
    data_label = "IEA CDD"
    download_raw_data(source_url, target_path, data_label)

    # IEA HDD
    source_url = "https://www.dropbox.com/s/np09gni0911ais6/HDD_18_IEA_20210406.zip?dl=1"
    target_path = os.path.join(project_root, r'data/raw/HDD_18_IEA_20210406.zip')
    data_label = "IEA HDD"
    download_raw_data(source_url, target_path, data_label)

    # Atalla CDD/HDD
    source_url = "https://www.dropbox.com/s/lbj680tg6x7f4jv/CDD_HDD_18_Atalla_20210406.zip?dl=1"
    target_path = os.path.join(project_root, r'data/raw/CDD_HDD_18_Atalla_20210406.zip')
